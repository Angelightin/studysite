...
FILES_MARKER="$STATE_DIR/.schema-files-ready"
...
if [ ! -f "$FILES_MARKER" ]; then
  node --import ./scripts/sites-env.mjs ./node_modules/wrangler/bin/wrangler.js d1 execute DB --local --config dist/server/wrangler.json --persist-to "$STATE_DIR" --file drizzle/0003_files_and_layout.sql
  touch "$FILES_MARKER"
fi
