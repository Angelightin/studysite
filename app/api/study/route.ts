...
export async function POST(request:Request){
  ...
  try{
    ...
    switch(a.action){
      ...
      case "deleteSubject":await db.batch([db.prepare("DELETE FROM tasks WHERE subject_id=?").bind(a.id),db.prepare("DELETE FROM subject_notes WHERE subject_id=?").bind(a.id),db.prepare("DELETE FROM lessons WHERE subject_id=?").bind(a.id),db.prepare("DELETE FROM resources WHERE subject_id=?").bind(a.id),db.prepare("DELETE FROM subjects WHERE id=? AND owner_id=?").bind(a.id,user.id)]);break;
